...
void Histogram::scretchHistogram(oneImage * img, int value)
{
	Pixel * pix;
	if (value <= 0) return;
...
void Histogram::stretchHistogramRGB(oneImage * img, int value)
{
	Pixel * pix;
	if (value <= 0) return;
	for (int i = 0; i < img->getHeight(); i++) {
...
oneImage * Histogram::getHistoRGB()
{
	int max = 0;
	int aHisto[256];
...
oneImage * Histogram::getHistoRed() 
{
	int max = 0;
	int aHisto[256];
...
oneImage * Histogram::getHistoGreen() 
{
	int max = 0;
	int aHisto[256];
...
oneImage * Histogram::getHistoBlue() 
{
	int max = 0;
	int aHisto[256];
...
Histogram::Histogram(oneImage * img)
{
	createHistogram(img);
}
void Histogram::createHistogram(oneImage * img)
{
	for (int i = 0; i <= 255; i++) {
		histoBlue[i] = 0;
		histoGreen[i] = 0;
